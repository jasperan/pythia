...
class CacheBadge(Static):
    ...
    def show_cache_hit(self, similarity: float, time_ms: int) -> None:
        ...
        text.append(f"from cache ({similarity:.2f} similarity)", style=f"{colors.SUCCESS}")
        text.append(f" \u00b7 {time_ms}ms", style=f"{colors.DIM}")
        self.update(text)

    def show_web_search(self, time_ms: int, sources_count: int) -> None:
        text = Text()
        text.append("  \u25cf ", style=f"bold {colors.INFO}")
        text.append(f"web search ({sources_count} sources)", style=f"{colors.INFO}")
